...
export default function FinalMixStudio() {
    ...
    // Build frame_id -> video_url map from project's completed video_tasks
    const videoTasks: VideoTask[] = currentProject?.video_tasks || [];
    const frameVideoMap = videoTasks.reduce((acc, task) => {
        if (task.status === 'completed' && task.video_url && task.frame_id) {
            acc[task.frame_id] = task.video_url;
        }
        return acc;
    }, {} as Record<string, string>);

    // Current frame based on playhead position
    const currentFrameIndex = Math.min(Math.floor(currentTime / 5), Math.max(frames.length - 1, 0));
    const currentFrame = frames[currentFrameIndex] || null;
    const currentFrameVideoUrl = currentFrame ? (currentFrame.video_url || frameVideoMap[currentFrame.id] || null) : null;

    useEffect(() => {
        setDuration(totalDuration);
    }, [frames]);

    // Control all tracks imperatively
    useEffect(() => {
        const playAll = () => {
            [videoRef, dialogueRef, sfxRef, bgmRef].forEach(ref => {
    ...
    // When frame changes while playing, restart all tracks
    useEffect(() => {
        if (!isPlaying) return;
        [videoRef, dialogueRef, sfxRef, bgmRef].forEach(ref => {
            if (ref.current) {
    ...
    // Apply volumes whenever they change
    useEffect(() => {
        if (videoRef.current) videoRef.current.volume = volumes.video;
        if (dialogueRef.current) dialogueRef.current.volume = volumes.voice;
        if (sfxRef.current) sfxRef.current.volume = volumes.sfx;
        if (bgmRef.current) bgmRef.current.volume = volumes.bgm;
    }, [volumes]);
            ...
            <div className="flex-1 flex border-b border-white/10 min-h-0">
                ...
                <div className="flex-1 bg-black/80 flex items-center justify-center relative p-8">
                    <div className="aspect-video bg-black/20 border border-white/10 rounded-lg w-full max-w-4xl flex items-center justify-center relative overflow-hidden shadow-2xl">
                        {/* Frame Preview */}
                        {currentFrameVideoUrl ? (
                            <video
                                ref={videoRef}
                                key={currentFrameVideoUrl}
                                src={getAssetUrl(currentFrameVideoUrl)}
                            ...
                            <img
                                key={currentFrame.image_url}
                                src={getAssetUrl(currentFrame.image_url)}
                                className="w-full h-full object-contain"
                            ...
                            <div className="text-gray-500 flex flex-col items-center gap-4">
                                ...
                                <div className="text-xs text-white/30">No media available</div>
                            </div>
                        )}

                        {/* Hidden Audio Tracks */}
                        {currentFrame?.audio_url && (
                            <audio ref={dialogueRef} key={`dialogue-${currentFrameIndex}`} src={getAssetUrl(currentFrame.audio_url)} />
                        )}
                        {currentFrame?.sfx_url && (
                            <audio ref={sfxRef} key={`sfx-${currentFrameIndex}`} src={getAssetUrl(currentFrame.sfx_url)} />
                        )}
                        {currentFrame?.bgm_url && (
                            <audio ref={bgmRef} key={`bgm-${currentFrameIndex}`} src={getAssetUrl(currentFrame.bgm_url)} />
                        )}

                        {/* Timecode Overlay */}
                        <div className="absolute top-4 right-4 bg-black/60 px-2 py-1 rounded font-mono text-xs text-white/60 backdrop-blur-sm">
