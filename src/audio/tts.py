...
class TTSProcessor:
    ...
    def synthesize(
        ...
        if 'intl' in ws_url:
            ...
            if resolved_model == 'cosyvoice-v2':
                raise ValueError(
                    f"Voice '{voice}' requires cosyvoice-v2 which is not available on the international endpoint. "
                    "Please select a v3 voice (e.g. longanyang or longanhuan)."
                )
    ...
    def list_voices():
        ...
        On international endpoint (DASHSCOPE_WEBSOCKET_URL contains 'intl'),
